...
export function EditProductPage() {
  ...
  const [originalSupplierCode, setOriginalSupplierCode] = useState<string>('');
  ...
  useEffect(() => {
    if (form.supplier_code && form.supplier_code !== originalSupplierCode && !form.isVariant) {
      generateSku(form.supplier_code);
    }
  }, [form.supplier_code, originalSupplierCode, form.isVariant]);
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (product) {
        ...
        const isVariant = product.sku.includes('.');
        let parentProductId = '';
        let variantSuffix = '';
        
        if (isVariant && product.metadata?.parentProductId) {
          parentProductId = product.metadata.parentProductId;
          
          // Extract suffix from SKU
          const suffixMatch = product.sku.match(/\.(\d+)$/);
          if (suffixMatch) {
            variantSuffix = `.${suffixMatch[1]}`;
          }
        }
        ...
        // Extract attribute groups from metadata if they exist
        const attributeGroups = product.metadata?.attributeGroups || [];
        ...
        setOriginalSupplierCode(product.supplier_code || '');
        ...
        setProductMedia(sortedMedia);
        setSelectedMediaFiles(sortedMedia.map(media => media.media_files));

        if (product.metadata?.discount_key) {
          setSelectedDiscountKey(product.metadata.discount_key);
        }
      }
    } catch (error) {
  ...
  const fetchParentProducts = async () => {
    try {
      ...
      setParentProducts(filteredProducts);
      setFilteredParentProducts(filteredProducts);
    } catch (error) {
  ...
  const handleIsVariantChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Reset parent product if toggling off
    if (!isVariant) {
      setForm(prev => ({
        ...prev,
        isVariant,
    ...
    try {
      const formData = {
        ...
        metadata: {
          ...
          variants_enabled: form.variants.length > 0,
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {Object.values(discountKeys).map((key) => (
                      <button
                        key={key.id}
