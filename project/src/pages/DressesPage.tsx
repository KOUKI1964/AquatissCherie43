...
export function DressesPage() {
  ...
  const fetchDiscountKeys = async () => {
    try {
      console.log('Fetching discount keys...');
      ...
      if (error) {
        console.error('Supabase error fetching discount keys:', error);
        throw error;
      }

      console.log('Discount keys fetched successfully:', data);
      ...
      // Just log the error and continue with empty discount keys
    }
  };
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (!categories || categories.length === 0) {
        setError('La catégorie "Robes" n\'est pas disponible pour le moment.');
        setIsLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-serif font-bold text-gray-900 mb-8">Robes</h1>
